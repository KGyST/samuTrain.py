# Data Flow Diagram for `scripts/try_calamari_mistral.py`
...
graph TD
    ...
    B --"data_dir: str"--> D[collect_chars_from_gt]
    B --"data_dir: str, charset: set[str]"--> E[load_pairs]
    ...
    subgraph UserProvidedData
        K1[User Images] --> E
        K2[User Labels] --> E
        K3[User Checkpoint] --> C
    end

    subgraph CalamariMistralScript
        B[main]
        C[load_checkpoint]
        C2[SavedCalamariModel]
        D[collect_chars_from_gt]
        E[load_pairs]
        K[train_batch]
        L[preprocess_batch]
        M[make_input_dict]
    ...
    L --"img: np.ndarray, target_height: int, max_width: int"--> M[make_input_dict]
    M --"image: np.ndarray, target_height: int, max_width: int"--> O[("dict: {'img': np.ndarray, 'img_len': np.ndarray}")]

    L --"codec: Codec, label: str"--> P[encode_gt]
    ...
    K --"y_pred: np.ndarray"--> AB[decode_prediction]
    H[("codec: Codec")] ----> AB
    AB --"y_pred: np.ndarray, codec: Codec"--> AC[("pred: str, conf: float")]

    B --"graph: TensorFlow Model"--> AD[save_checkpoint]
    G ----> AD
    H[("codec: Codec")] ----> AD
    B --"output_dir: str, step: int"--> AD
    AD --"graph: TensorFlow Model, params: TrainerParams, codec: Codec, output_dir: str, step: int"--> AE[save_checkpoint]
